...
export class CartService implements ICartService {
    ...
    createItem =
        async (userId: number, productId: number, quantity: number): Promise<CartFullResponse> => {
            const cart: CartFullEntity = await this.cartRepository.createItem(userId, productId, quantity);
    ...
    updateItemQuantity =
        async (userId: number, productId: number, quantity: number): Promise<CartFullResponse> => {
            const cart: CartFullEntity = await this.cartRepository.updateItemQuantity(userId, productId, quantity);
